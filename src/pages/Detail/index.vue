...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters(['categoryView','skuInfo','spuSaleAttrList','filterSpuSaleAttrListColor','filterSpuSaleAttrListVersion']),
      //给子组件的数据
      skuImageList(){
        //如果服务器的数据没有回来，skuInfo这个对象是个空对象，属性skuImageList为undefined
    ...
    methods: {
      ...
      async addShopCart(){
        ...
        /* 
        当前这里是派发一个action,也是服务器发请求，判断加入购物车是成功的还是失败了，进行相应的操作。
        调用仓库的addOrUpdateShopCart，这个方法加上了asyc,返回一定是Promise
        Promise要么成功要么失败
        */
       try {
        ...
        //产品信息的数据【比较复杂skuInfo】,通过会话存储(不持久化，会话结束数据消失)
        //无论是本地存储还是会话存储，一般都是存储字符串
        //把它转为字符串
        sessionStorage.setItem("SKUINFO",JSON.stringify(this.skuInfo));
        sessionStorage.setItem("FILTERSPUSALEATTRLISTCOLOR",JSON.stringify(this.filterSpuSaleAttrListColor));
        sessionStorage.setItem("FILTERSPUSALEATTRLISTVERSION",JSON.stringify(this.filterSpuSaleAttrListVersion));
